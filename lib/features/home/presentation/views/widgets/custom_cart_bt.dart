  ...
  Widget build(BuildContext context) {
    return BlocBuilder<CartItemCubit, CartItemState>(
      builder: (context, state) {
        ...
        return CustomButton(
            text: localization.checkoutPrice(context.watch<CartCubit>().cartEntity.calculateTotalPrice().toStringAsFixed(2)),
            onPressed: (){
              if (cartItems.cartItems.isNotEmpty) {
                Navigator.pushNamed(context, CheckoutView.routeName, arguments: cartItems);
              }else{
                showErrorBar(context, localization.noProductsInCart, backgroundColor: Colors.red, durationInSec: 1);
              }
            }
        );
      },
